...
async def _send_reference_library(message: Message, context: AppContext, user_id: int) -> None:
    ...
    if not tasks:
        await message.answer(
            "<b>Мои референсы</b>\n\n"
            "Здесь появятся уникальные наборы фото после первой генерации.",
    ...
    await message.answer(
        "<b>Мои референсы</b>\n\n"
        "Выберите сохранённый набор. Бот восстановит фото, модель, промпт, формат и качество, "
        "а перед запуском повторно проверит текущую цену, лимиты и баланс.",
