...
function applyPermalinkMode(cards) {
  ...
  grid.querySelector(".community-permalink-bar")?.remove();
  cards.forEach((card) => {
    card.hidden = false;
    card.classList.remove("is-permalink-post");
  });
  if (!requestedPostId || !cards.length) {
    return;
  }
  ...
  if (target) {
    ...
    target.classList.add("is-permalink-post");
    target.hidden = false;
  }
  const mosaic = grid.querySelector(".community-mosaic");
  const anchor = mosaic || grid.firstElementChild;
  if (anchor) {
    anchor.before(permalinkBar(Boolean(target)));
  } else {
    grid.prepend(permalinkBar(Boolean(target)));
  }
}
