    ...
    async with engine.connect() as conn:
        ...
        try:
            async with session_factory() as session:
                ...
                ok, reason = await share_task_to_feed(session, task_id=feed_task.id, user_id=regular_user.id)
                assert ok, reason
                feed_items = await get_feed_tasks(session, limit=5)
                assert any(item.id == feed_task.id for item in feed_items)
                likes, is_new = await like_feed_task(session, task_id=feed_task.id, user_id=regular_user.id)
                assert likes == 1 and is_new
                likes, is_new = await like_feed_task(session, task_id=feed_task.id, user_id=regular_user.id)
                assert likes == 1 and not is_new
                shares = await increment_feed_share(session, feed_task.id)
                assert shares == 1
                assert await remove_task_from_feed(session, task_id=feed_task.id, user_id=regular_user.id)
                ...
                await session.flush()
                ok, reason = await share_task_to_feed(session, task_id=derivative.id, user_id=regular_user.id)
...
async def amain() -> None:
    ...
    print("admin smoke/regression passed")
